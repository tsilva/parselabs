...
if len(unknown_names) > 0:
    ...
    # Group by raw lab_name to see patterns
    name_counts = unknown_names.groupby(['lab_name', 'lab_type']).size().reset_index(name='count')
    ...
    for _, row in name_counts.iterrows():
        print(f"  [{row['lab_type']}] {row['lab_name']:60s} ({row['count']} occurrences)")
else:
...
if len(unknown_units) > 0:
    ...
    # Group by lab_name and unit to see patterns
    unit_counts = unknown_units.groupby(['lab_name', 'unit', 'lab_type']).size().reset_index(name='count')
    ...
    for _, row in unit_counts.iterrows():
        print(f"  [{row['lab_type']}] {row['lab_name']:50s} | Unit: {row['unit']:15s} ({row['count']} occurrences)")
else:
...
print(f"Unique lab names in data: {df['lab_name'].nunique()}")
