"""Schema validation for ``lab_specs.json``.
...
class LabSpecsValidator:
    ...
    def _validate_lab_entries(self) -> None:
        ...
        self._validate_critical_loinc_invariants(loinc_codes)
    ...
    def _validate_critical_loinc_invariants(self, loinc_codes: dict[str, list[str]]) -> None:
        """Validate known critical mappings and incompatible code sharing."""

        for lab_name, expected_code in self.CRITICAL_LOINC_CODES.items():
            spec = self.config.get(lab_name)
            if not isinstance(spec, dict):
                continue

            actual_code = spec.get("loinc_code")
            if actual_code != expected_code:
                self.errors.append(f"{lab_name} code should be {expected_code}, got {actual_code}")

        for loinc_code, lab_names in loinc_codes.items():
            for first, second in self.FORBIDDEN_LOINC_SHARING:
                has_first = any(first in lab_name for lab_name in lab_names)
                has_second = any(second in lab_name for lab_name in lab_names)
                if has_first and has_second:
                    self.errors.append(f"LOINC {loinc_code} incorrectly shared between {first} and {second}: {lab_names}")
