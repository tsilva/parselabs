...
Usage:
    parselabs admin lab-specs <command> [options]
...
Examples:
    ...
    parselabs admin lab-specs build-conversions
    parselabs admin lab-specs build-ranges
"""
...
from parselabs.paths import get_lab_specs_path, get_prompts_dir, get_source_lab_specs_path
...
def load_context(profile_name: str) -> RuntimeContext:
    """Load API-backed runtime state for one profile."""
...
def cmd_sort(args):
    ...
    lab_specs_path = get_source_lab_specs_path()
...
def cmd_build_conversions(args):
    ...
    if client is None:
        raise RuntimeError("API-backed lab-spec maintenance requires an OpenAI client.")
...
Examples:
    parselabs admin lab-specs sort
    parselabs admin lab-specs fix-encoding --backup
